...
func Parse(data []byte, opts ...model.Option) (qb.Query, error) {
	...
	return model.ParseDocument(payload, opts...)
}

// Marshal lowers a query into ordered JSON bytes.
func Marshal(query qb.Query, opts ...model.Option) ([]byte, error) {
	document, err := model.BuildDocument(query, model.TransportJSON, opts...)
	if err != nil {
		return nil, err
	}
