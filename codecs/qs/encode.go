...
func Encode(query qb.Query, opts ...codecs.Option) (string, error) {
	document, err := docmodel.BuildDocument(query, docmodel.TransportQueryString, opts...)
	if err != nil {
		return "", err
	}
...
func EncodeValues(query qb.Query, opts ...codecs.Option) (url.Values, error) {
	document, err := docmodel.BuildDocument(query, docmodel.TransportQueryString, opts...)
	if err != nil {
		return nil, err
	}
